...
def make_background(screen_w: int, screen_h: int):
    x = np.linspace(0.0, 1.0, screen_w, dtype=np.float32)[None, :]
    y = np.linspace(0.0, 1.0, screen_h, dtype=np.float32)[:, None]
    blue = 28.0 + 44.0 * (1.0 - y) + 14.0 * x
    green = 18.0 + 26.0 * (1.0 - y) + 6.0 * x
    red = 14.0 + 22.0 * x + 4.0 * (1.0 - y)
...
def draw_progress_bar(canvas, x: int, y: int, width: int, height: int, progress: float, fill_color: tuple):
    ...
    cv2.rectangle(canvas, (x, y), (x + width, y + height), (46, 54, 74), -1)
    cv2.rectangle(canvas, (x, y), (x + width, y + height), (86, 98, 128), 1)
...
def draw_metric_card(canvas, x: int, y: int, width: int, height: int, title: str, value: str, accent: tuple):
    ...
    put_text(canvas, title, (x + 18, y + 28), 0.55, (175, 190, 220), 1)
    put_text(canvas, value, (x + 18, y + 72), 0.95, (245, 248, 252), 2)
...
def render_intro_screen(base_bg: np.ndarray, total_points: int, samples_per_point: int):
    ...
    cv2.rectangle(canvas, (56, 54), (840, 350), (16, 20, 32), -1)
    cv2.rectangle(canvas, (56, 54), (840, 350), (0, 176, 255), 2)
    put_text(canvas, "Eye Tracker Calibration", (82, 110), 1.2, (246, 248, 252), 2)
    put_text(canvas, "Fullscreen calibration with hidden webcam preview.", (82, 152), 0.72, (182, 196, 224), 2)
    put_text(canvas, "1. Sit naturally and keep your head steady.", (82, 212), 0.72, (232, 236, 242), 2)
    put_text(canvas, "2. Follow each orange dot until the capture finishes.", (82, 248), 0.72, (232, 236, 242), 2)
    put_text(canvas, "3. Quality and average accuracy are shown on-screen.", (82, 284), 0.72, (232, 236, 242), 2)
    draw_metric_card(canvas, card1_x, 70, 190, 96, "Dots", str(total_points), (0, 176, 255))
    draw_metric_card(canvas, card2_x, 70, 220, 96, "Samples / Dot", str(samples_per_point), (92, 206, 132))
    put_text(canvas, "SPACE to begin", (84, base_bg.shape[0] - 74), 0.84, (250, 251, 253), 2)
    put_text(canvas, "ESC to cancel", (340, base_bg.shape[0] - 74), 0.84, (184, 198, 226), 2)
    return canvas
...
def render_calibration_screen(
    ...
    cv2.rectangle(canvas, (54, 52), (650, 334), (16, 20, 32), -1)
    cv2.rectangle(canvas, (54, 52), (650, 334), (0, 176, 255), 2)
    put_text(canvas, "Eye Tracker Calibration", (78, 92), 1.0, (246, 248, 252), 2)
    put_text(canvas, f"{phase_text} dot {current_idx}/{total_points}", (78, 128), 0.72, (184, 198, 226), 2)
    draw_progress_bar(canvas, 78, 148, 540, 18, progress, (0, 176, 255))
    put_text(canvas, "Overall progress", (78, 142), 0.46, (154, 170, 198), 1)
    draw_progress_bar(canvas, 78, 178, 540, 14, sample_progress, (92, 206, 132))
    put_text(canvas, "Current dot capture", (78, 173), 0.46, (154, 170, 198), 1)

    draw_metric_card(canvas, 78, 214, 156, 84, "Samples", f"{collected}/{samples_per_point}", (92, 206, 132))
    draw_metric_card(canvas, 254, 214, 156, 84, "Dot Quality", format_score(dot_quality), (0, 176, 255))
    ...
    status_color = (92, 206, 132) if face_ready else (0, 176, 255)
    cv2.rectangle(canvas, (screen_w - 310, 248), (screen_w - 60, 332), (16, 20, 32), -1)
    cv2.rectangle(canvas, (screen_w - 310, 248), (screen_w - 60, 332), status_color, 2)
    put_text(canvas, "Face Status", (screen_w - 292, 276), 0.55, (175, 190, 220), 1)
    put_text(canvas, "Ready" if face_ready else "Waiting", (screen_w - 292, 314), 0.92, (246, 248, 252), 2)

    dot_outer = 44
    dot_mid = 24
    cv2.circle(canvas, target, dot_outer, (78, 112, 255), 2)
    cv2.circle(canvas, target, dot_mid, (0, 176, 255), -1)
    cv2.circle(canvas, target, 7, (250, 250, 252), -1)
    cv2.line(canvas, (target[0] - 34, target[1]), (target[0] + 34, target[1]), (224, 232, 242), 2)
    cv2.line(canvas, (target[0], target[1] - 34), (target[0], target[1] + 34), (224, 232, 242), 2)

    put_text(canvas, status_text, (80, screen_h - 62), 0.75, (236, 239, 244), 2)
    put_text(canvas, "ESC to cancel calibration", (80, screen_h - 28), 0.56, (170, 182, 206), 1)
    return canvas
...
def render_summary_screen(
    ...
    canvas = base_bg.copy()
    screen_h, screen_w = canvas.shape[:2]
    cv2.rectangle(canvas, (54, 52), (screen_w - 54, screen_h - 92), (16, 20, 32), -1)
    cv2.rectangle(canvas, (54, 52), (screen_w - 54, screen_h - 92), (92, 206, 132), 2)

    put_text(canvas, "Calibration Complete", (82, 100), 1.1, (246, 248, 252), 2)
    put_text(canvas, f"Model saved to {output_path.name} ({feature_mode})", (82, 138), 0.68, (184, 198, 226), 2)

    draw_metric_card(canvas, 82, 174, 220, 96, "Average Accuracy", f"{average_accuracy:.1f}%", (92, 206, 132))
    draw_metric_card(canvas, 326, 174, 220, 96, "Average Error", f"{average_error:.0f} px", (0, 176, 255))
    draw_metric_card(canvas, 570, 174, 220, 96, "Capture Quality", f"{average_quality:.1f}%", (255, 171, 72))

    put_text(canvas, "Per-dot accuracy", (82, 322), 0.72, (235, 239, 244), 2)
    ...
    for row_idx, metric in enumerate(left_items):
        ...
        put_text(canvas, text, (92, start_y + row_idx * row_gap), 0.66, (230, 235, 242), 2)

    for row_idx, metric in enumerate(right_items):
        text = f"P{metric['index']:02d}   {metric['accuracy']:.1f}%   {metric['error_px']:.0f}px"
        put_text(canvas, text, (screen_w // 2 + 60, start_y + row_idx * row_gap), 0.66, (230, 235, 242), 2)

    put_text(canvas, str(output_path), (82, screen_h - 118), 0.52, (166, 180, 205), 1)
    put_text(canvas, "Press any key to close", (82, screen_h - 74), 0.78, (246, 248, 252), 2)
    return canvas
