...
def main():
    ...
    parser.add_argument("--smooth", type=float, default=0.07, help="0..1, higher is snappier")
    parser.add_argument("--sensitivity", type=float, default=0.62, help="final cursor range multiplier; keep near 1.0 so all screen edges remain reachable")
    parser.add_argument("--stability-window", type=int, default=17, help="number of recent predictions used for median stabilization")
    parser.add_argument("--steady-radius-px", type=float, default=55.0, help="keep the gaze point steadier while predictions stay within this radius")
    parser.add_argument("--response-px", type=float, default=340.0, help="distance over which the filter ramps from steady to responsive")
    parser.add_argument("--head-assist", type=float, default=None, help="0..1 blend weight for head/face assist in hybrid mode")
    parser.add_argument("--x-gain", type=float, default=1.28, help="horizontal range multiplier for reaching left and right screen edges")
    parser.add_argument("--y-gain", type=float, default=1.22, help="vertical range multiplier for reaching top and bottom screen edges")
    parser.add_argument(
        "--control-mouse",
    ...
    parser.add_argument("--cursor-deadzone-px", type=float, default=26.0, help="ignore tiny cursor moves under this pixel distance")
    parser.add_argument("--edge-zone-ratio", type=float, default=0.24, help="outer screen band ratio where edge assist starts pushing outward")
    parser.add_argument("--edge-boost-px", type=float, default=260.0, help="maximum outward push near the screen edges")
    ...
    parser.add_argument("--snap-radius-px", type=float, default=170.0, help="max distance for snapping to a nearby clickable control")
    parser.add_argument("--snap-hold", type=float, default=1.10, help="seconds to hold onto a snapped control after acquisition")
    parser.add_argument("--snap-dwell", type=float, default=0.04, help="seconds gaze must stay steady before snapping to a control")
    parser.add_argument("--snap-dwell-radius-px", type=float, default=55.0, help="allowed movement during snap dwell acquisition")
    parser.add_argument("--snap-unlock-distance-px", type=float, default=240.0, help="release a snapped target early if gaze moves this far away")
    ...
    parser.add_argument("--blink-recovery", type=float, default=0.28, help="seconds to hold cursor steady after a blink ends")
